...
const AddFamilyMember = ({
    ...
    const handleSubmit = (e) => {
        ...
        if (value === 'Registered-Family-Member' && (!newMember.email && !newMember.mobileNumber)) {
            setError(true);
            return;
        }
        axios
            .patch('http://localhost:8002/family-members/' + userId, { member: newMember })
            .then((data) => {
                setIsOpen(false);
    ...
    return (
        <div>
            <Dialog open={isOpen} onClose={setIsOpen}>
                ...
                <DialogContent>
                    <FamilyMemberOptions handleChange={handleChange} value={value} />
                    <form autoComplete='off' onSubmit={handleSubmit}>
                        ...
                        {error && <Typography color='error' variant="h5"> Please enter a valid email or mobile number </Typography>}
                    </form>
                </DialogContent>
            </Dialog>
        </div>
