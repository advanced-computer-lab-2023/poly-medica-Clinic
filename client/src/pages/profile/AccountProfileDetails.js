...
export const AccountProfileDetails = () => {
	...
	useEffect(() => {
		const getPatientsURL = 'http://localhost:8001/doctor/65228c39b5e8bef73ca7ca85';
		// let user;
    
		axios.get(getPatientsURL, { withCredentials: true }).then((response) => {
	...
	const handleSubmit = (event) => {
		...
		setLoading(true);
		const getPatientsURL = 'http://localhost:8001/doctors/65228c39b5e8bef73ca7ca85';
		// let user;
    
		axios.patch(getPatientsURL, values, { withCredentials: true }).then((response) => {
			const values = response.data.doctor;
			console.log('values', values);
			Swal.fire({
		...
		<form autoComplete="off"  onSubmit={handleSubmit}>
			<Card>
				...
				<CardActions sx={{ justifyContent: 'flex-end' }}>
					<Button variant="contained" onClick={handleSubmit} disabled={loading}>Save details</Button>
				</CardActions>
			</Card>
		</form>
	);
