...
export const user = (app) => {
    ...
    app.delete('/users/:id', async (req, res) =>{
        try{
            ...
            res.status(SERVER_ERROR_REQUEST_CODE_500).send({message: "coudn't delete the user"})
        }
    })
    ...
    app.post('/doctors', async (req, res) =>{
        try{
            ...
            res.status(SERVER_ERROR_REQUEST_CODE_500).send({errMessage:"coudn't add the doctor"})
        }
    })
    ...
    app.post('/login', async (req, res) =>{
        try{
            ...
            res.send({id:logedinUser._id, name:logedinUser.userName, type: logedinUser.type });
        } catch(err){
            res.status(BAD_REQUEST_CODE_400).send({message: err.message});
        }
    })
